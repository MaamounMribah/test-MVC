...
$(document).ready(function () {
    ...
    $("select.FiltreListe").change(function () {
        ...
        $.ajax({
            url: '/InterrogerStock/GetArticleInformationByReference?RefArticle=' + IdArticle + "&NumSerie=" + NumSerie,
...
function SetData(Globaldata) {
    ...
    if (Globaldata.data.MouvementVente.length != 0)
    {
    ...
    if (Globaldata.data.MouvementAchat.length != 0)
    {
    ...
    if (Globaldata.data.DemandeApprosViewModels.length != 0)
    {
    ...
    if (Globaldata.data.MouvementTransferArticleViewModels.length != 0)
    {
    ...
    if (Globaldata.data.DemandeTransfertArticleViewModels.length != 0) {
...
function GetSerie(SerieData,typetab)
{
    $("#ListSeries tbody tr").remove();
    
    for (var i = 0; i < SerieData.length; i++)
    {
        var aa = SerieData[i].NumSerie == undefined ? SerieData[i] : SerieData[i].NumSerie;
        var cc = SerieData[i].Type == undefined ? "" : SerieData[i].Type ;
        $('#ListSeries tbody').append('<tr><td>' + aa + '</td><td>' + cc + '</td></tr>');
...
$(document).ready(function () {
    ...
    var TableAllowed = ["DemandeTransfertTab", "MouvementTransferTab", "DemandeApproTab", "BonSortieTab", "StockTab", "ReserveMateriel"];
